  ...
  <div>
      ...
      <div class="">
        ...
        <div class="flex">
          ...
          <div class="flex-auto px-1 py-1">
            <p class="font-bold text-normal sm:text-lg text-gray-700">{{ product.product.name }}</p>
            <p class="text-sm text-gray-700"><span v-if="product.name">{{ product.name }}&nbsp;</span>{{ product.vol }}
              {{ product.measure_type }}</p>
          </div>
        </div>

        <div class="self-start px-1 py-1 text-gray-700">
          <b>{{ product.total }}</b>
        </div>

        <div class="flex">
          <div class="flex-auto">
            <div class="self-start px-1 py-1">
              <div class="control">
          ...
          <div class="flex-auto">
            <div class="self-start px-1 py-1 text-right">
    ...
    <div class="hidden sm:block">
      ...
      <div class="flex flex-col sm:flex-row content-around sm:justify-between">
        ...
        <div class="sm:order-1 w-32 flex-none px-2 py-2 align-middle text-gray-800">
          <img class="rounded"
               :src="`https://api.chito-ra.ru/storage/${product.product.images[0].name}`"
               alt="">
        </div>
        ...
        <div class="order-1 sm:order-2 w-64 align-left sm:align-middle px-1 sm:px-4 py-1 sm:py-3 align-middle">
          <p class="font-bold text-normal sm:text-lg text-gray-700">
            <nobr>{{ product.product.name }}</nobr>
          </p>
          <p class="text-sm text-gray-700"><span v-if="product.name">{{ product.name }}&nbsp;</span>{{ product.vol }} {{
            product.measure_type }}</p>
        </div>
